...
export default function Centres() {
  ...
  const searchFields = [
    { name: 'name', label: 'Centre Name', type: 'text', gridSize: 3 },
    {
      name: 'school',
  ...
  const formFields = [
    ...
    { name: 'code', label: 'Centre Code', type: 'text', required: true },
    {
      name: 'school',
      label: 'School / Business Unit',
      type: 'select',
    ...
    (values) => {
      try {
        if (editItem) {
          ...
          enqueueSnackbar('Centre updated successfully', { variant: 'success' });
        } else {
          storageService.create(STORAGE_KEY, values);
          enqueueSnackbar('Centre created successfully', { variant: 'success' });
        }
        setFormOpen(false);
        setEditItem(null);
        refresh();
  ...
  const handleDeleteConfirm = useCallback(() => {
    try {
      ...
      enqueueSnackbar('Centre deleted successfully', { variant: 'success' });
      setDeleteOpen(false);
      setDeleteItem(null);
      refresh();
    } catch (error) {
  ...
  const columns = [
    ...
    { field: 'name', headerName: 'Centre Name', flex: 1.3, minWidth: 160 },
    { field: 'school', headerName: 'School / BU', flex: 1, minWidth: 140 },
    { field: 'address', headerName: 'Address', flex: 1.3, minWidth: 180 },
  ...
  return (
    <Box>
      <PageHeader
        title="Centres Management"
        breadcrumbs={breadcrumbs}
        actionLabel="Add Centre"
        actionIcon={<AddIcon />}
        onAction={handleAdd}
      />
      ...
      <DataTable rows={data} columns={columns} exportFilename="centres" />

      <FormDialog
        open={formOpen}
        onClose={() => setFormOpen(false)}
        onSubmit={handleFormSubmit}
        title={editItem ? 'Edit Centre' : 'Add Centre'}
        fields={formFields}
