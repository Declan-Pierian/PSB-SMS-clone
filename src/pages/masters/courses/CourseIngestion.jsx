...
export default function CourseIngestion() {
  const { enqueueSnackbar } = useSnackbar();
  const [uploading, setUploading] = useState(false);
  const [fileName, setFileName] = useState('');
  const [statusMessage, setStatusMessage] = useState(null);
  const [history, setHistory] = useState(() => storageService.getAll('courseIngestions'));

  const refreshHistory = useCallback(() => {
    setHistory(storageService.getAll('courseIngestions'));
  }, []);
  ...
  const handleUpload = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const ingestionRecord = {
        id: uuidv4(),
        batchId: `CB-${Date.now().toString().slice(-8)}`,
        fileName,
      ...
      storageService.create('courseIngestions', ingestionRecord);
      setUploading(false);
      setFileName('');
      refreshHistory();
      ...
      setStatusMessage({
        ...
        text: `File processed: ${successCount} courses imported successfully${failedCount > 0 ? `, ${failedCount} failed` : ''}.`,
      });

      enqueueSnackbar(`Course ingestion completed - ${successCount} of ${totalRecords} records processed`, {
  ...
  return (
    <Box>
      <PageHeader title="Course Ingestion" breadcrumbs={breadcrumbs} />

      <Paper sx={{ p: 3, mb: 3 }}>
        <Typography variant="h6" gutterBottom>Upload Course Data</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Upload a CSV or Excel file containing course data. Download the template for the correct format.
        </Typography>
      ...
      <DataTable rows={history} columns={columns} exportFilename="course-ingestion-history" />
    </Box>
  );
}