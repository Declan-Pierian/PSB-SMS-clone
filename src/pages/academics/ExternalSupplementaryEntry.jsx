...
const breadcrumbs = [{ label: 'Academics', path: '/academics' }, { label: 'External Supplementary' }];

export default function ExternalSupplementaryTestDataEntry() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const items = JSON.parse(localStorage.getItem('sms_externalSupplementary') || '[]');
    items.push({ id: uuidv4(), ...form, createdDate: new Date().toISOString().split('T')[0] });
    localStorage.setItem('sms_externalSupplementary', JSON.stringify(items));
  ...
  return (
    <Box>
      <PageHeader title="External Supplementary Test Data Entry" breadcrumbs={breadcrumbs} />
      <Paper sx={{ p: 3 }}>
