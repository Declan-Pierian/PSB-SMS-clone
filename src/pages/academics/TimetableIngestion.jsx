...
const breadcrumbs = [{ label: 'Academics', path: '/academics' }, { label: 'Timetable Ingestion' }];

export default function TimetableIngestion() {
  ...
  return (
    <Box>
      <PageHeader title="Timetable Ingestion" breadcrumbs={breadcrumbs} />
      <Paper sx={{ p: 3, mb: 3 }}>
      ...
      {results.length > 0 && <DataTable rows={results} columns={columns} exportFilename="timetable-import" />}
    </Box>
  );
}
